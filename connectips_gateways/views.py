...
class ConnectIpsTokenView(APIView):
    def post(self, request):
        try:
            ...
            pfx_path = config.creditor_pfx_file.path
            # pfx_path = os.path.join(settings.MEDIA_ROOT, f"CREDITOR_{tenant_header}.pfx")
            if not pfx_path:
                return Response({"error": f"PFX certificate not found for tenant"}, status=404)
...
class ConnectIpsSuccessUrl(BaseCallback):
    def get(self, request):
        ...
        pfx_path = config.creditor_pfx_file.path
        if not pfx_path:
            return Response({"error": f"PFX file not found for tenant."}, status=404)
